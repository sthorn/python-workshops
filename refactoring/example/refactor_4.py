...
def send_to_grafana(timestamp, hostname, metric_name, metric_value):
    """Send metric to Grafana"""
    cmd = f'echo cluster.monitoring.{hostname}.{metric_name} {metric_value} {timestamp} | nc -w1 172.16.48.100 2003'
    run(cmd, shell=True)
...
def main():
    while True:
        ...
        send_to_grafana(timestamp, hostname, metric_name='mem_used', metric_value=mem_used)
